...
def _teacher_signal_class(*, row: dict[str, Any], winner: str) -> str:
    raw_utility = float(row.get("raw_finance_utility", 0.0))
...
class ExtractedFinanceTraceRow:
    source_report: str
...
def extract_finance_trace_bank(*, report_paths: list[str], min_utility_delta: float | None = None) -> dict[str, Any]:
    traces: list[ExtractedFinanceTraceRow] = []
    report_count = 0
    skipped_rows = 0
    ...
    for raw_path in report_paths:
        ...
        for index, row in enumerate(report.get("rows", []), start=1):
            winner = _winner(float(row.get("raw_finance_utility", 0.0)), float(row.get("memla_finance_utility", 0.0)))
            ...
            traces.append(
                ExtractedFinanceTraceRow(
                    ...
                    raw_finance_utility=round(float(row.get("raw_finance_utility", 0.0)), 4),
                    ...
                    memla_finance_utility=round(float(row.get("memla_finance_utility", 0.0)), 4),
...
def render_finance_trace_bank_markdown(report: dict[str, Any]) -> str:
    lines = [
        "# Finance Pre-Trade Trace Bank",
        "",
