...
def _clean_google_answer_text(text: str) -> str:
...
def _extract_google_ai_overview_from_html(html: str) -> dict[str, Any]:
    match = re.search(r"AI Overview", str(html or ""), flags=re.IGNORECASE)
    if not match:
        return {}
    window = str(html or "")[match.start() : match.start() + 8000]
    stop_markers = [
        re.search(r"(?is)<h3\b", window),
        re.search(r"(?is)<div[^>]+class=\"[^\"]*\bg\b", window),
        re.search(r"(?is)<g-card\b", window),
    ]
    stops = [marker.start() for marker in stop_markers if marker]
    if stops:
        window = window[: min(stops)]
    answer = _clean_google_answer_text(window)
...
def _extract_google_featured_snippet_from_html(html: str) -> dict[str, Any]:
    patterns = (
    ...
    for pattern in patterns:
        for raw in re.findall(pattern, str(html or "")):
            answer = _clean_google_answer_text(raw)
            if not answer or _looks_like_low_signal_web_text(answer):
                continue
...
def _fetch_google_answer_surface(query: str) -> dict[str, Any]:
    ...
    if not html:
        return {}
    for extractor in (_extract_google_ai_overview_from_html, _extract_google_featured_snippet_from_html):
        payload = extractor(html)
        if payload:
...
def _render_memla_web_answer(
    *,
    ...
    if str(source_kind or "").strip() in {"google_ai_overview", "google_featured_snippet", "google_answer_box"}:
        follow_up = ""
